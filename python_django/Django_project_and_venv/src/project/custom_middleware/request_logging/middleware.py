...
class LoggingMiddleware(object):
    ...
    def _log_resp(self, level, response, logging_context):
        self.logger.log2(level, response, logging_context)
        if re.match('^application/json', response.get('Content-Type', ''), re.I):
            self.logger.log2(level, response._headers, logging_context)
            if response.streaming:
                ...
                self.logger.log2(level, self._chunked_to_max(response.content),
                                logging_context)
